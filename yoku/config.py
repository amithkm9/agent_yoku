...
# `from yoku.config import tickets_collection`. New code should prefer
...
def __getattr__(name: str):
    if name in _LAZY_STORAGE_EXPORTS:
        from yoku.storage import mongo as _mongo

        value = getattr(_mongo, name)
        globals()[name] = value  # cache for subsequent accesses
        return value
    raise AttributeError(f"module 'yoku.config' has no attribute {name!r}")
